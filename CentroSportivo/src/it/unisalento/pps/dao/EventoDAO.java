...
public class EventoDAO {
	...
	public ArrayList<Evento> getCorsiByIdDisciplina(int idDisciplina) {
		...
		for(int i=0;i<result.size();i++) {
			...
			long millisecondi_fine = dataFine.getTimeInMillis();
			Date data_fine = new Date(millisecondi_fine);
			corso = new Evento(Integer.parseInt(result.get(i)[0]),data_inizio,data_fine,result.get(i)[2],Integer.parseInt(result.get(i)[3]),Integer.parseInt(result.get(i)[4]),Integer.parseInt(result.get(i)[5]),idDisciplina);
			corsi.add(corso);
		}
		return corsi;
	}
	...
	public ArrayList<Evento> getEventiByIdDisciplina(int idDisciplina) {
		ArrayList<String[]> result=DbConnection.getInstance().eseguiQuery("select * from EVENTO where (Disciplina = \""+idDisciplina+"\") and (tipo = 1) and (tipo = 2)");
	...
	public ArrayList<Evento> getEventiCorsiByIdDisciplina(int idDisciplina) {
		ArrayList<String[]> result=DbConnection.getInstance().eseguiQuery("select * from EVENTO where (Disciplina = \""+idDisciplina+"\")");
