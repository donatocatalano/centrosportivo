	...
	public ElencoIstruttori(Responsabile responsabile) {
		...
		if(istruttori.size()>0) {
			...
			for(int i=0;i<istruttori.size();i++) {
				...
				JLabel istruttore = new JLabel("ID ISTRUTTORE: " +istruttori.get(i).getIdIstruttore()+"    NOME: " +istruttori.get(i).getCognome()+"    COGNOME: " +istruttori.get(i).getNome()+"    USERNAME: "+istruttori.get(i).getUsername()+"    PASSWORD: "+istruttori.get(i).getPassword()+"     ");
